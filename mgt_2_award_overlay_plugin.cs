...
namespace MGT2AwardOverlay
{
    ...
    public sealed class AwardOverlayPlugin : BaseUnityPlugin
    {
        ...
        private string _devStatus = "初期化待ち...";
        private string _awardStatus = "初期化待ち...";
        ...
        private void Awake()
        {
            _enabled = Config.Bind("General", "Enabled", true, "有効/無効");
            _devToggleKey = Config.Bind("General", "DevToggleKey", KeyCode.F7, "F7 開発予測表示");
            _awardToggleKey = Config.Bind("General", "AwardToggleKey", KeyCode.F8, "F8 受賞表示");
            _fontSize = Config.Bind("UI", "FontSize", 14, "文字サイズ");

            _devWindowX = Config.Bind("UI", "DevWindowX", 20, "開発窓 X");
            _devWindowY = Config.Bind("UI", "DevWindowY", 20, "開発窓 Y");
            _devWindowW = Config.Bind("UI", "DevWindowW", 780, "開発窓 W");
            _devWindowH = Config.Bind("UI", "DevWindowH", 920, "開発窓 H");

            _awardWindowX = Config.Bind("UI", "AwardWindowX", 830, "受賞窓 X");
            _awardWindowY = Config.Bind("UI", "AwardWindowY", 20, "受賞窓 Y");
            _awardWindowW = Config.Bind("UI", "AwardWindowW", 760, "受賞窓 W");
            _awardWindowH = Config.Bind("UI", "AwardWindowH", 920, "受賞窓 H");
        ...
        private void DrawDevWindow(int id)
        {
            ...
            GUILayout.Label("=== 開発中ゲームデバッグ ===", _headerStyle);
            GUILayout.Label("状態: " + _devStatus, _labelStyle);
            ...
            if (_mainScript == null)
            {
                GUILayout.Label("mainScript 未解決", _labelStyle);
            ...
            if (_devRows.Count == 0)
            {
                GUILayout.Label("<color=yellow>表示対象が0件です</color>", _labelStyle);
            }
            else
            {
        ...
        private void DrawAwardWindow(int id)
        {
            ...
            GUILayout.Label("=== 受賞候補オーバーレイ ===", _headerStyle);
            GUILayout.Label("状態: " + _awardStatus, _labelStyle);
            ...
            GUILayout.BeginVertical(_boxStyle);
            GUILayout.Label("myID=" + myId + "  date=" + year + "/" + month + "/" + week, _labelStyle);
            GUILayout.Label("対象期間: " + displayStartYear + "/12/1 ～ " + year + "/" + month + "/" + week, _labelStyle);
            ...
            DrawAwardSection("映像賞候補", _gfxAwardRows, "GFX");
            DrawAwardSection("サウンド賞候補", _soundAwardRows, "SND");
            DrawAwardSection("GOTY候補", _gotyRows, "TOTAL");
            DrawAwardSection("ワースト候補", _worstRows, "LOW");
            DrawCompanySection("最優秀開発賞候補", _studioAwardRows, "STUDIO");
            DrawCompanySection("パブリッシャー賞候補", _publisherAwardRows, "PUBLISHER");
            DrawAwardSection("自社発売済みゲーム", _selfRows, "SELF");
        ...
        private void DrawAwardSection(string title, List<AwardGameRow> rows, string mode)
        {
            ...
            if (rows.Count == 0)
            {
                GUILayout.Label("(なし)", _labelStyle);
                GUILayout.EndVertical();
                GUILayout.Space(6);
                return;
            }
        ...
        private void DrawDevRow(DevTaskRow r)
        {
            ...
            GUILayout.Label("総合予測: <b>" + r.Total.ToString("0.0") + "</b>", _labelStyle);

            float need = 80f - r.Total;
            if (need > 0)
                GUILayout.Label("<color=red>80まで +" + need.ToString("0.0") + "</color>", _labelStyle);
            else
                GUILayout.Label("<color=cyan>80達成済</color>", _labelStyle);
            ...
            if (!_compact)
            {
                ...
                {
                    GUILayout.Label("=== 要因 ===", _headerStyle);
                ...
                if (r.Tips.Count > 0)
                {
                    GUILayout.Label("=== 改善優先度 ===", _headerStyle);
        ...
        private void RefreshDevRows()
        {
            try
            {
                ...
                if (rooms == null)
                {
                    _devStatus = "arrayRoomScripts 未取得";
                    _devDebugInfo = "";
                    return;
                }
            ...
            {
                _devStatus = "更新エラー: " + ex.GetType().Name + " / " + ex.Message;
                _devDebugInfo = "";
                Logger.LogError(ex);
                ResetResolvedReferences();
            }
        ...
        private void Analyze(DevTaskRow r, object g)
        {
            ...
            if (ctrlDef > 0)
            {
                r.Elements.Add(new DeltaElement("CONTROL不足", -ctrlDef));
                r.Tips.Add("CONTROL優先強化 +" + ctrlDef.ToString("0.0"));
            }
            ...
            if (gameSize == 0)
            {
                r.Elements.Add(new DeltaElement("サイズ減点", -3f));
            }

            bool oldEngine = GetBool(g, "engineIsOld");
            if (oldEngine)
            {
                r.Elements.Add(new DeltaElement("旧エンジン", -5f));
            }

            float bugs = ToFloat(GetFieldOrPropertyValue(g, "points_bugs"));
            if (bugs > 0)
            {
                r.Elements.Add(new DeltaElement("バグ", -bugs));
            }

            float marketing = ToFloat(GetFieldOrPropertyValue(g, "marketingBonus"));
            if (Math.Abs(marketing) > 0.001f)
            {
                r.Elements.Add(new DeltaElement("マーケ補正", marketing));
            }

            if (GetBool(g, "typ_contractGame"))
            {
                r.Elements.Add(new DeltaElement("契約ゲーム", -2f));
            }
        ...
        private void RefreshAwardRows()
        {
            try
            {
                ...
                if (allGames == null)
                {
                    _awardStatus = "arrayGamesScripts 未取得";
                    _awardDebugInfo = "";
                    return;
                }
            ...
            {
                _awardStatus = "更新エラー: " + ex.GetType().Name + " / " + ex.Message;
                _awardDebugInfo = "";
                Logger.LogError(ex);
            }
        }
        ...
        private int GetReleaseWeek(object g)
        {
            ...
            // 発売年・発売月が取れているのに週だけ無い場合は、1週として扱う
            if (v <= 0)
            {
