...
namespace MGT2AwardOverlay
{
    ...
    public sealed class AwardOverlayPlugin : BaseUnityPlugin
    {
        ...
        private string _devStatus = "Waiting for initialization...";
        private string _awardStatus = "Waiting for initialization...";
        ...
        private void Awake()
        {
            _enabled = Config.Bind("General", "Enabled", true, "Enable / Disable");
            _devToggleKey = Config.Bind("General", "DevToggleKey", KeyCode.F7, "F7 Toggle Dev Prediction");
            _awardToggleKey = Config.Bind("General", "AwardToggleKey", KeyCode.F8, "F8 Toggle Awards");
            _fontSize = Config.Bind("UI", "FontSize", 14, "Font Size");

            _devWindowX = Config.Bind("UI", "DevWindowX", 20, "Dev Window X");
            _devWindowY = Config.Bind("UI", "DevWindowY", 20, "Dev Window Y");
            _devWindowW = Config.Bind("UI", "DevWindowW", 780, "Dev Window Width");
            _devWindowH = Config.Bind("UI", "DevWindowH", 920, "Dev Window Height");

            _awardWindowX = Config.Bind("UI", "AwardWindowX", 830, "Award Window X");
            _awardWindowY = Config.Bind("UI", "AwardWindowY", 20, "Award Window Y");
            _awardWindowW = Config.Bind("UI", "AwardWindowW", 760, "Award Window Width");
            _awardWindowH = Config.Bind("UI", "AwardWindowH", 920, "Award Window Height");
        ...
        private void DrawDevWindow(int id)
        {
            ...
            GUILayout.Label("=== Development Debug ===", _headerStyle);
            GUILayout.Label("Status: " + _devStatus, _labelStyle);
            ...
            if (_mainScript == null)
            {
                GUILayout.Label("mainScript unresolved", _labelStyle);
            ...
            if (_devRows.Count == 0)
            {
                GUILayout.Label("<color=yellow>No entries to display</color>", _labelStyle);
            }
            else
            {
        ...
        private void DrawAwardWindow(int id)
        {
            ...
            GUILayout.Label("=== Awards Overlay ===", _headerStyle);
            GUILayout.Label("Status: " + _awardStatus, _labelStyle);
            ...
            GUILayout.BeginVertical(_boxStyle);
            GUILayout.Label("myID=" + myId + "  date=" + year + "/" + month + "/" + week, _labelStyle);
            GUILayout.Label("Period: " + displayStartYear + "/12/1 - " + year + "/" + month + "/" + week, _labelStyle);
            ...
            DrawAwardSection("Best Graphics Candidates", _gfxAwardRows, "GFX");
            DrawAwardSection("Best Sound Candidates", _soundAwardRows, "SND");
            DrawAwardSection("Game of the Year Candidates", _gotyRows, "TOTAL");
            DrawAwardSection("Worst Game Candidates", _worstRows, "LOW");
            DrawCompanySection("Best Developer Candidates", _studioAwardRows, "STUDIO");
            DrawCompanySection("Best Publisher Candidates", _publisherAwardRows, "PUBLISHER");
            DrawAwardSection("Released Self Games", _selfRows, "SELF");
        ...
        private void DrawDevRow(DevTaskRow r)
        {
            ...
            GUILayout.Label("Overall Prediction: <b>" + r.Total.ToString("0.0") + "</b>", _labelStyle);

            float need = 80f - r.Total;
            if (need > 0)
                GUILayout.Label("<color=red>Need +" + need.ToString("0.0") + " to reach 80</color>", _labelStyle);
            else
                GUILayout.Label("<color=cyan>80 reached</color>", _labelStyle);
            ...
            if (!_compact)
            {
                ...
                {
                    GUILayout.Label("<color=yellow>Unexplained loss (skill/compatibility suspected): -"
                ...
                {
                    GUILayout.Label("=== Factors ===", _headerStyle);
                ...
                if (r.Tips.Count > 0)
                {
                    GUILayout.Label("=== Improvement Priority ===", _headerStyle);
        ...
        private void RefreshDevRows()
        {
            try
            {
                ...
                if (_mainScript == null)
                {
                    _devStatus = "mainScript unresolved";
                    _devDebugInfo = "";
                    return;
                }
                ...
                if (rooms == null)
                {
                    _devStatus = "arrayRoomScripts not found";
                    _devDebugInfo = "";
                    return;
                }
            ...
            {
                _devStatus = "Update error: " + ex.GetType().Name + " / " + ex.Message;
                _devDebugInfo = "";
                Logger.LogError(ex);
                ResetResolvedReferences();
            }
        ...
        private void Analyze(DevTaskRow r, object g)
        {
            ...
            if (!targetOk)
            {
                r.Elements.Add(new DeltaElement("Target group mismatch", -3f));
                r.Tips.Add("Check target audience compatibility");
            }

            if (!genreComboOk)
            {
                r.Elements.Add(new DeltaElement("Genre combination mismatch", -3f));
                r.Tips.Add("Check main/sub genre compatibility");
            }

            if (!mainThemeOk)
            {
                r.Elements.Add(new DeltaElement("Main theme mismatch", -3f));
                r.Tips.Add("Check theme compatibility with genre");
            }

            if (!subThemeOk)
            {
                r.Elements.Add(new DeltaElement("Sub theme mismatch", -1.5f));
                r.Tips.Add("Check sub-theme compatibility");
            }

            if (r.EstimatedGenrePenalty > 0.1f)
            {
                r.Elements.Add(new DeltaElement("Genre skill penalty (estimated)", -r.EstimatedGenrePenalty));
                r.Tips.Add("Genre skill: main Lv" + r.MainGenreLevel + " / sub Lv" + r.SubGenreLevel);
            }

            if (r.EstimatedThemePenalty > 0.1f)
            {
                r.Elements.Add(new DeltaElement("Theme skill penalty (estimated)", -r.EstimatedThemePenalty));
                r.Tips.Add("Theme skill: main Lv" + r.MainThemeLevel + " / sub Lv" + r.SubThemeLevel);
            }

            float gpDef = 80f - r.Gp;
            if (gpDef > 0)
            {
                r.Elements.Add(new DeltaElement("Gameplay below target", -gpDef));
                r.Tips.Add("Improve Gameplay +" + gpDef.ToString("0.0"));
            }

            float gfxDef = 80f - r.Gfx;
            if (gfxDef > 0)
            {
                r.Elements.Add(new DeltaElement("Graphics below target", -gfxDef));
                r.Tips.Add("Improve Graphics +" + gfxDef.ToString("0.0"));
            }

            float ctrlDef = 80f - r.Ctrl;
            if (ctrlDef > 0)
            {
                r.Elements.Add(new DeltaElement("Control below target", -ctrlDef));
                r.Tips.Add("Improve Control +" + ctrlDef.ToString("0.0"));
            }

            float sndDef = 80f - r.Snd;
            if (sndDef > 0)
            {
                r.Elements.Add(new DeltaElement("Sound below target", -sndDef));
                r.Tips.Add("Improve Sound +" + sndDef.ToString("0.0"));
            }

            int gameSize = GetInt(g, "gameSize");
            if (gameSize == 0)
            {
                r.Elements.Add(new DeltaElement("Game size penalty", -3f));
            }

            if (GetBool(g, "engineIsOld"))
            {
                r.Elements.Add(new DeltaElement("Old engine penalty", -5f));
            }
            ...
            {
                r.Elements.Add(new DeltaElement("Marketing bonus", marketing));
            }

            if (GetBool(g, "typ_contractGame"))
            {
                r.Elements.Add(new DeltaElement("Contract game penalty", -2f));
            }

            if (GetInt(g, "portID") != -1)
            {
                r.Elements.Add(new DeltaElement("Port penalty", -3f));
            }
            ...
            if (r.ActReviewTotal > 0)
            {
                ...
                {
                    r.Elements.Add(new DeltaElement("Unexplained loss (skill/compatibility suspected)", -r.UnexplainedPenalty));
                    r.Tips.Add("Hidden penalties exist");
                }
            }
        }
        ...
        private void RefreshAwardRows()
        {
            try
            {
                ...
                if (_mainScript == null)
                {
                    _awardStatus = "mainScript unresolved";
                    _awardDebugInfo = "";
                    return;
                }
                ...
                if (allGames == null)
                {
                    _awardStatus = "arrayGamesScripts not found";
                    _awardDebugInfo = "";
                    return;
                }
            ...
            {
                _awardStatus = "Update error: " + ex.GetType().Name + " / " + ex.Message;
                _awardDebugInfo = "";
                Logger.LogError(ex);
            }
        }
        ...
        private int GetReleaseWeek(object g)
        {
            ...
            // If year and month are known but week is missing, treat it as week 1.
            if (v <= 0)
            {
        ...
        private float GetStudioPoints(object g)
        {
            ...
            // fallback: use reviewTotal as a provisional point value
